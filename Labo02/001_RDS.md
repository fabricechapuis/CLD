...
```bash
[INPUT]
aws ec2 create-security-group --description "Allow access to RDS" --group-name "SG-PRIVATE-DRUPAL-DEVOPSTEAM04-RDS" --vpc-id <vpc-id> --tag-specifications "ResourceType=security-group,Tags=[{Key=Name,Value=SG-PRIVATE-DRUPAL-DEVOPSTEAM04-RDS}]"
