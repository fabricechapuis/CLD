...
```bash
[INPUT]
aws ec2 create-security-group --description "Allow access to RDS" --group-nameo "SG-PRIVATE-DRUPAL-DEVOPSTEAM<XX>-RDS" --vpc-id <vpc-id> --tag-specifications "ResourceType=security-group,Tags=[{Key=Name,Value=SG-PRIVATE-DRUPAL-DEVOPSTEAM<XX>-RDS}]"
...
[INPUT]
aws ec2 authorize-security-group-ingress --group-id <group-id> --protocol tcp --port 3306 --source-group <group-id DRUPAL>
...
[OUTPUT]
{
    ...
    "SecurityGroupRules": [
        {
            "SecurityGroupRuleId": "sgr-0ad8c8139064ec1e2",
